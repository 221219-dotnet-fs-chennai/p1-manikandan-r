...
async function filteroption() {
    ...
    let skill = document.getElementById("skillbox").value;

    if (city == null || city == "") {
        city = "ex:_chennai_or_delhi";
    }
    if (skill == null || skill == "") {
        skill = "ex:_python_or_java";
    }

    console.log(city);
    console.log(skill);

    await fetch("https://localhost:7234/Api/User/TrainerUsingFilter?" + new URLSearchParams
        ({
            city: city,
            skill: skill
        }), {
